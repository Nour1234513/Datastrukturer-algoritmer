...
long long unsigned int theTime=0;
...
void simulate(int antalKassor){
    ...
    while (infil >> customer){
       ...
       // if(que.empty()){
            //que.push(Event(customer.m_name,customer.m_arrival+customer.m_timeSpent)) ;
       // }
       // if(!que.empty()){
            //que.push(Event(customer.m_name,customer.m_timeSpent+que.top().m_timeToLeave));
